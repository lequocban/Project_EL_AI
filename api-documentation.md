...
```json
{
  "code": 201,
  "success": true,
  "message": "Nộp bài luyện đọc thành công",
  "data": {
    "practiceId": "uuid",
    "score": 80,
    "totalQuestions": 5,
...
```json
{
  "code": 200,
  "success": true,
  "message": "Lấy lịch sử luyện đọc thành công",
  "data": {
    "items": [
      {
        "id": "uuid",
        "lessonId": "uuid",
        "lessonTitle": "The Impact of Climate Change",
        "score": 80,
        "totalQuestions": 5,
        "correctAnswers": 4,
        "createdAt": "2026-05-10T00:00:00Z",
        "completeAt": "2026-05-10T00:05:00Z"
      }
    ],
    "total": 25,
    "page": 1,
    ...
    "results": [
      {
        ...
        "correctAnswer": "B",
        "isCorrect": true,
        "explain": "The passage clearly states..."
      },
      {
        "questionId": "uuid-2",
...
```json
{
  "code": 201,
  "success": true,
  "message": "Tạo bài luyện nghe bằng AI thành công",
  "data": {
    "id": "uuid",
    "title": "Business Phone Call",
...
```json
{
  "code": 201,
  "success": true,
  "message": "Tạo bài luyện nghe thành công",
  "data": {
    "id": "uuid",
    "title": "Daily English Conversation",
    "audioUrl": "https://example.com/audio.mp3",
...
```json
{
  "code": 200,
  "success": true,
  "message": "Lấy danh sách bài luyện nghe của tôi thành công",
  "data": {
    "items": [
      {
        "id": "uuid",
...
```json
{
  "code": 200,
  "success": true,
  "message": "Lấy danh sách bài luyện nghe công khai thành công",
  "data": {
    "items": [
      {
        "id": "uuid",
...
```json
{
  "code": 200,
  "success": true,
  "message": "Lấy chi tiết bài luyện nghe thành công",
  "data": {
    "id": "uuid",
    "title": "Business Phone Call",
    "audioUrl": "https://example.com/audio.mp3",
...
```json
{
  "code": 200,
  "success": true,
  "message": "Cập nhật bài luyện nghe thành công",
  "data": {
    "id": "uuid",
    "title": "Updated Title",
    "audioUrl": "https://example.com/new-audio.mp3",
...
```json
{
  "code": 200,
  "success": true,
  "message": "Xóa bài luyện nghe thành công",
  "data": null
}
```
...
```json
{
  "code": 200,
  "success": true,
  "message": "Gửi yêu cầu công khai bài luyện nghe thành công",
  "data": {
    "id": "uuid",
    "title": "Business Phone Call",
    "status": "req_public",
...
```json
{
  "code": 200,
  "success": true,
  "message": "Chuyển bài luyện nghe sang chế độ riêng tư thành công",
  "data": {
    "id": "uuid",
    "title": "Business Phone Call",
    "status": "private",
...
**Phản hồi mẫu cho `POST /listening-lessons/:lessonId/questions` (201):**
```json
{
  "code": 201,
  "success": true,
  ...
  "data": {
    ...
    "option_d": "...",
    "correct_answer": "B",
    "explain": "The conversation clearly states...",
    "createdAt": "2026-05-15T00:00:00Z"
  }
}
```

**Phản hồi mẫu cho `GET /listening-lessons/:lessonId/questions` (200):**
```json
{
  "code": 200,
  "success": true,
...
**Phản hồi mẫu cho `POST /listening-lessons/:lessonId/practice/submit` (201):**
```json
{
  "code": 201,
  "success": true,
  "message": "Nộp bài luyện nghe thành công",
  "data": {
    "practiceId": "uuid",
    "score": 80,
    "totalQuestions": 5,
...
**Phản hồi mẫu cho `GET /listening-lessons/practice/history` (200):**
```json
{
  "code": 200,
  "success": true,
  "message": "Lấy lịch sử luyện nghe thành công",
  "data": {
    "items": [
      {
        "id": "uuid",
        "lessonId": "uuid",
        "lessonTitle": "Business Phone Call",
        "score": 80,
        "totalQuestions": 5,
        "correctAnswers": 4,
        "createdAt": "2026-05-10T00:00:00Z",
...
**Phản hồi mẫu cho `GET /listening-lessons/practice/:practiceId` (200):**
```json
{
  "code": 200,
  "success": true,
...
```json
{
  "code": 201,
  "success": true,
  "message": "Tạo yêu cầu kiểm duyệt thành công",
  "data": {
    "id": "uuid",
    "contentType": "vocabulary_set",
    "contentId": "uuid",
...
| Mã | Thông báo | Điều kiện |
|----|-----------|-----------|
| 409 | "Yêu cầu kiểm duyệt đã tồn tại cho nội dung này" | Đã gửi yêu cầu rồi |
| 400 | "Loại nội dung không hợp lệ" | contentType không đúng |
...
```json
{
  "code": 200,
  "success": true,
  "message": "Lấy danh sách yêu cầu kiểm duyệt thành công",
  "data": {
    "items": [
      {
        "id": "uuid",
...
> **Lưu ý:** Tất cả các endpoint trong Admin API yêu cầu quyền Admin (`verifyToken` + `requireAuth` + `requireAdmin`) hoặc Content Manager/Admin (`verifyToken` + `requireAuth` + `requireManagerOrAdmin`).

### 5.1. Admin Auth - Đăng Nhập Quản Trị

#### 5.1.1. Đăng nhập Admin
...
**Mô tả:** Đăng nhập với tài khoản admin. Trả về Access Token (JWT) và Refresh Token.
...
|---------|------|-----------|---------|--------|
| `page` | number | `1` | - | Số trang |
| `limit` | number | `15` | `100` | Số item trên trang |
| `keyword` | string | `""` | - | Tìm kiếm theo tên, email, username |
| `role` | string | `""` | - | Lọc theo vai trò: `user`, `content_manager`, `admin` |
| `status` | string | `""` | - | Lọc theo trạng thái: `active`, `inactive` |
| `sortField` | string | `"created_at"` | - | Trường sắp xếp: `created_at` (ngày tạo), `email` (email) |
| `sortOrder` | string | `"desc"` | - | Thứ tự: `asc`, `desc` |

**Ví dụ:**

```
GET /api/v1/admin/users?role=user&email=gmail.com&status=active&sortOrder=desc
...
```json
{
  "code": 200,
  "success": true,
  "message": "Cấp vai trò content_manager cho người dùng thành công",
  "data": {
    "userId": "uuid-user-1",
    "role": "content_manager",
    "action": "grant"
...
```json
{
  "code": 200,
  "success": true,
  "message": "Lấy danh sách bộ từ vựng chờ duyệt thành công",
  "data": {
    "items": [
      {
        "id": "uuid-vs-1",
        "title": "IELTS Vocabulary - Topic: Education",
...
```json
{
  "code": 200,
  "success": true,
  "message": "Duyệt công khai bộ từ vựng thành công",
  "data": {
    "id": "uuid-vs-1",
    "status": "public"
  }
...
```json
{
  "reason": "Nội dung không phù hợp với tiêu chuẩn cộng đồng"
}
```

| Trường | Kiểu | Bắt buộc | Mô tả |
|--------|------|-----------|--------|
| `reason` | string | **Có** | Lý do từ chối |
...
```json
{
  "code": 200,
  "success": true,
  "message": "Từ chối bộ từ vựng thành công",
  "data": {
    "id": "uuid-vs-1",
    "status": "rejected",
    "reason": "Nội dung không phù hợp với tiêu chuẩn cộng đồng"
...
```json
{
  "code": 200,
  "success": true,
  "message": "Duyệt công khai bài luyện đọc thành công",
  "data": {
    "id": "uuid-rl-1",
    "status": "public"
  }
...
```json
{
  "reason": "Nội dung bài đọc không đạt chất lượng"
}
```
...
```json
{
  "code": 200,
  "success": true,
  "message": "Từ chối bài luyện đọc thành công",
  "data": {
    "id": "uuid-rl-1",
    "status": "rejected",
    "reason": "Nội dung bài đọc không đạt chất lượng"
...
```json
{
  "code": 200,
  "success": true,
  "message": "Lấy danh sách bài luyện nghe chờ duyệt thành công",
  "data": {
    "items": [
      {
        "id": "uuid-ll-1",
        "title": "BBC News - Climate Change",
        "description": "Bài nghe tin tức BBC về biến đổi khí hậu",
        "audioUrl": "https://example.com/audio/bbc-climate.mp3",
        "transcript": "Today we're going to discuss...",
...
```json
{
  "code": 200,
  "success": true,
  "message": "Duyệt công khai bài luyện nghe thành công",
  "data": {
    "id": "uuid-ll-1",
    "status": "public"
  }
...
```json
{
  "reason": "Chất lượng audio không đạt yêu cầu"
}
```
...
```json
{
  "code": 200,
  "success": true,
  "message": "Từ chối bài luyện nghe thành công",
  "data": {
    "id": "uuid-ll-1",
    "status": "rejected",
    "reason": "Chất lượng audio không đạt yêu cầu"
...
```json
{
  "code": 200,
  "success": true,
  "message": "Lấy danh sách yêu cầu kiểm duyệt bài luyện đọc thành công",
  "data": {
    "items": [
      {
        "id": "uuid-mod-2",
        "contentType": "reading_lesson",
        "contentId": "uuid-rl-1",
        "contentTitle": "The Impact of Technology on Education",
        "status": "pending",
        "submitter": {
          "id": "uuid-user-1",
          "username": "johndoe",
...
```json
{
  "code": 200,
  "success": true,
  "message": "Lấy danh sách yêu cầu kiểm duyệt bài luyện nghe thành công",
  "data": {
    "items": [
      {
        "id": "uuid-mod-3",
        "contentType": "listening_lesson",
        "contentId": "uuid-ll-1",
        "contentTitle": "BBC News - Climate Change",
        "status": "pending",
        "submitter": {
          "id": "uuid-user-1",
          "username": "johndoe",
...
```json
{
  "code": 200,
  "success": true,
  "message": "Lấy chi tiết yêu cầu kiểm duyệt thành công",
  "data": {
    "id": "uuid-mod-1",
    "contentType": "vocabulary_set",
    "contentId": "uuid-vs-1",
    "contentTitle": "IELTS Vocabulary - Topic: Education",
    "contentDetails": {
      "title": "IELTS Vocabulary - Topic: Education",
      "description": "Từ vựng IELTS chủ đề Giáo dục",
      "wordCount": 50,
...
```json
{
  "code": 200,
  "success": true,
  "message": "Phê duyệt yêu cầu kiểm duyệt thành công",
  "data": {
    "id": "uuid-mod-1",
    "status": "approved",
    "moderator": {
...
```json
{
  "code": 200,
  "success": true,
  "message": "Chỉnh sửa bộ từ vựng thành công",
  "data": {
    "id": "uuid-vs-1",
    "title": "IELTS Vocabulary - Topic: Education (Updated)",
    "description": "Từ vựng IELTS chủ đề Giáo dục - phiên bản cập nhật",
    "status": "pending",
...
```json
{
  ...
  "example": "The school has revised its curriculum."
}
```
...
|---------|------|-----------|--------|
| `wordId` | uuid | **Có** | ID của từ cần xóa |
...
```json
{
  "code": 200,
  "success": true,
  "message": "Chỉnh sửa bài luyện đọc thành công",
  "data": {
    "id": "uuid-rl-1",
    "title": "The Impact of Technology on Education (Updated)",
    "content": "The rapid advancement of technology has transformed modern education...",
    "status": "pending",
...
```json
{
  "code": 200,
  "success": true,
  "message": "Chỉnh sửa câu hỏi đọc hiểu thành công",
  "data": {
    "id": "uuid-q-rl-1",
    "question": "What is the main topic of the passage?",
    "allAnswers": {
...
```json
{
  "code": 200,
  "success": true,
  "message": "Chỉnh sửa bài luyện nghe thành công",
  "data": {
    "id": "uuid-ll-1",
    "title": "BBC News - Climate Change (Updated)",
    "audioUrl": "https://example.com/audio/bbc-climate-v2.mp3",
    "transcript": "Today we're going to discuss climate change and its impact...",
    "status": "pending",
...
```json
{
  "code": 200,
  "success": true,
  "message": "Chỉnh sửa câu hỏi nghe hiểu thành công",
  "data": {
    "id": "uuid-q-ll-1",
    "question": "What is the main topic of this news report?",
    "allAnswers": {
      "a": "Weather forecast",
      "b": "Climate change and its global impact",
      "c": "Economic news",
      "d": "Sports updates"
    },
    "correctAnswer": "B"
